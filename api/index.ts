...
router.post('/scenarios/:id/generate', (req, res) => {
    try {
        ...
        const paramsRows = db.prepare('SELECT block_type, data FROM scenario_parameters WHERE scenario_id = ?').all(id) as any[];
        const params: Record<string, any> = {};
        paramsRows.forEach(p => params[p.block_type] = JSON.parse(p.data));

        const wbMarket = XLSX.utils.book_new();
        const ratesData = [['Term', 'Base Rate', 'Shocked Rate'], ['1M', 0.03, 0.03 + (params.rates?.shift_bp || 0) / 10000]];
        XLSX.utils.book_append_sheet(wbMarket, XLSX.utils.aoa_to_sheet(ratesData), "Interest Rates");

        const marketBuffer = XLSX.write(wbMarket, { type: 'buffer', bookType: 'xlsx' });
        res.json({ files: [{ name: 'market_data_template.xlsx', content: marketBuffer.toString('base64') }] });
    } catch (e: any) {
        res.status(500).json({ error: e.message });
    }
});

// Explicitly handle both /api and root routes to be resilient to Vercel rewrites
app.use('/api', router);
app.use('/', router);
