...
export function ScenarioBuilder({ scenarioId, onBack }: ScenarioBuilderProps) {
  ...
  if (scenario?.risk_types) {
    ...
    scenario.risk_types.forEach((risk: RiskType) => {
              ...
              >
                ...
                {activeBlock === 'rates' && (
                  <RatesBlock
                    ...
                    onChange={(data: any) => updateParameters('rates', data)}
                  />
                )}
                {activeBlock === 'behavior' && (
                  <BehaviorBlock
                    data={scenario.parameters?.behavior}
                    onChange={(data: any) => updateParameters('behavior', data)}
                  />
                )}
                {activeBlock === 'inflation' && (
                  <InflationBlock
                    data={scenario.parameters?.inflation}
                    onChange={(data: any) => updateParameters('inflation', data)}
                  />
                )}
                {activeBlock === 'macro' && (
                  <MacroBlock
                    data={scenario.parameters?.macro}
                    onChange={(data: any) => updateParameters('macro', data)}
                  />
                )}
                {activeBlock === 'csrbb' && (
                  <CSRBBBlock
                    data={scenario.parameters?.csrbb}
                    onChange={(data: any) => updateParameters('csrbb', data)}
                  />
                )}
                {activeBlock === 'liquidity' && (
                  <LiquidityBlock
                    data={scenario.parameters?.liquidity}
                    onChange={(data: any) => updateParameters('liquidity', data)}
                  />
                )}
                {activeBlock === 'fx' && (
                  <FXBlock
                    data={scenario.parameters?.fx}
                    onChange={(data: any) => updateParameters('fx', data)}
                  />
                )}
              </motion.div>
            ) : (
